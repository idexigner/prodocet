...
<div class="container-xxl flex-grow-1 container-p-y">
    @if(_has_permission('roles.view'))
        <div class="pagetitle row mt-4 mb-4">
            ...
            <div class="col-lg-6">
                @if(_has_permission('roles.create'))
    ...
    @if(_has_permission('roles.create') || _has_permission('roles.edit'))
        <!-- Add/Edit Role Modal -->
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadPermissions(selectedPermissions = []) {
                console.log('Loading permissions with selected:', selectedPermissions);
                $.get("{{ route('roles.create') }}", function(response) {
                    if (response.success) {
                        ...
                        // After setting the HTML, ensure checkboxes are properly checked
                        if (selectedPermissions.length > 0) {
                            console.log('Setting checkboxes for permissions:', selectedPermissions);
                            selectedPermissions.forEach(function(permission) {
                                var checkbox = $('input[name="permissions[]"][value="' + permission + '"]');
                                console.log('Setting checkbox for permission:', permission, 'Found:', checkbox.length);
            ...
            $(document).on('click', '.delete-btn', function(e) {
                ...
                if (confirm("{{ __('roles.confirm_delete') }}")) {
                    $.ajax({
                        url: "{{ route('roles.destroy', ['id' => '__id__']) }}".replace('__id__', id),
            ...
            $(document).on('click', '.restore-btn', function(e) {
                ...
                if (confirm("{{ __('roles.confirm_restore') }}")) {
                    $.ajax({
                        url: "{{ route('roles.restore', ['id' => '__id__']) }}".replace('__id__', id),             
                        type: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            var updateUrl = "{{ route('roles.update', ['id' => '__id__']) }}".replace('__id__', id);

                            $('#update_form_btn').attr('data-url', updateUrl);
                            
                            // Load permissions with selected permissions
                            var selectedPermissions = role.permissions || [];
